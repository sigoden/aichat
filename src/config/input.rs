...
pub struct Input {
    text: String,
    medias: Vec<(String, Option<String>)>,
}
...
impl Input {
            ...
            match resolve_path(&file_item) {
                Some(file_path) => {
                    ...
                    if is_image_ext(&file_path) {
                        ...
                        medias.push((data_url, Some(file_path.display().to_string())))
                    } else {
    ...
    pub fn render(&self) -> String {
        ...
        let files: Vec<String> = self
            ...
            .iter()
            .cloned()
            .map(|(url, path)| path.unwrap_or(url))
            .collect();
    ...
    pub fn to_message_content(&self) -> MessageContent {
        if self.medias.is_empty() {
            ...
            let mut list: Vec<MessageContentPart> = self
                ...
                .iter()
                .cloned()
                .map(|(url, path)| MessageContentPart::ImageUrl {
